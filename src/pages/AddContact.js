...
const AddContact = () => {
  ...
  const imagePicker = async (e) => {
    ...
    try {
      const file = e.target.files[0]; ///////// Pick up the file nd 0 means very first of it

      var metadata = {
        contentType: file.type,
      };

      let resizedImage = await readAndCompressImage(file, imageConfig); /// resized our image

      const storageRef = await firebase.storage().ref(); // grap the reference from our database
      var uploadTask = storageRef ////////// Here we are uploading image
        .child("images/" + file.name) // if image/ is there then it grab the obj or if its not there then it create one for us
        .put(resizedImage, metadata); /// there are more methods then put... here we are sending img nd meta data... its compulsar to pass meta or something
      //// In place of "file.name" ppl use random name to avoid duplicate file name

      uploadTask.on(
        //////// There are events in firebase which we can use... here we use "on" event
        firebase.storage.TaskEvent.STATE_CHANGED, /// when anything change in our storage then we get snapshot back
        (snapshot) => {
          setIsUploading(true); // this is so that we can show a spinner till our data is updating
          var progress = /// to keep the track of our progress
            (snapshot.bytesTransferred / snapshot.totalBytes) * 100; ///Its mentioned in docs also

          switch (
            snapshot.state /// we checking our current stage of snapshot
          ) {
        ...
        () => {
          /////// whatever futher we want to do after that we can do in this callback
          uploadTask.snapshot.ref
            .getDownloadURL() /////// we getting download link of imge we uploaded
            .then((downloadURL) => {
              setDownloadUrl(downloadURL); //// stored the downloadurl in our state
            })
  ...
  const addContact = async () => {
    ...
    try {
      firebase
        .database()
        .ref("contacts/" + v4()) //// this will create contacts directory nd inside it new directory generated by v4 randomly
        .set({
          name, ////// we normally need to pass key value pairs but in recent version of JS we can directly pass.
          email,
          phoneNumber,
          address,
          picture: downloadUrl,
  ...
  const updateContact = async () => {
    ...
    try {
      ...
      console.error(error);
      toast("Error in Updation", { type: "error" });
    }
  };
