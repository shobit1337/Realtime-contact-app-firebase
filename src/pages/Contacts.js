...
const Contacts = () => {
  ...
  const AddContact = () => {
    //dispatch to send user to add contact screen
    dispatch({
      /////////// this dispatch is just an precaution.... this might encounter a bug
  ...
  return (
    <Container className="mt-4">
      ...
      {contacts.length === 0 && !isLoading ? (
        <div className="Center text-large text-primary"> NO CONTACTS</div>
      ) : (
        <ListGroup>
          {Object.entries(contacts).map((
            [key, value] ///////////////// Read about Object.entries on mozilla docs of js
          ) => (
